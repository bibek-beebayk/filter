...
class FilterViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    # queryset = Video.objects.all().select_related('country').prefetch_related('tags', 'genres').filter(tags__in = filter_params['tags'], genres__in = filter_params['genres'], country__in = filter_params['countries']).distinct()
